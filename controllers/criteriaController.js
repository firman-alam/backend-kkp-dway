...
const deleteCriteria = async (req, res) => {
  ...
  try {
    const [result] = await pool
      .promise()
      .query(
        "DELETE FROM kriteria WHERE id_kriteria = ? AND id_divisi = ? AND created_by = ?",
        [id_kriteria, id_divisi, userId]
      );

    if (result.affectedRows > 0) {
      res
        .status(200)
        .json({ status: true, message: "Kriteria berhasil dihapus" });
    } else {
      res
        .status(404)
        .json({ status: false, message: "Kriteria tidak ditemukan" });
    }
  } catch (error) {
    console.error(error);
    res.status(500).send("Internal Server Error");
